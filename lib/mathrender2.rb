...
module Mathrender2
  ...
  class << self
    ...
    def render_png(mathml, file_path='sample.png')
      ...
      output = Phantomjs.run File.join(File.dirname(__FILE__), 'mathrender2', 'mathml2png.js'), mathml, path
      png_data = File.read(path)
      File.delete(path)
      png_data
    end

    def render_svg(mathml)
      Phantomjs.run File.join(File.dirname(__FILE__), 'mathrender2', 'mathml2svg.js'), mathml
    end
...
puts Mathrender2.render_svg(math)